          ...
          <div className="hidden md:flex items-center gap-1">
            {navLinks.map((link) => (
                ...
                >
                  {link.label}
                  {isActive(link.path) && (
                      <motion.div layoutId="nav-indicator" className="absolute bottom-0 left-1/2 -translate-x-1/2 w-6 h-0.5 bg-brew-green rounded-full" />
                  )}
                </Link>
            ))}
          </div>
                  ...
                  {navLinks.map((link) => (
                      <Link
                          ...
                          className={`px-4 py-2.5 rounded-lg text-sm font-medium transition-colors ${
                              isActive(link.path) ? 'bg-brew-green text-white' : 'hover:bg-muted text-foreground'
                          }`}
                      >
                        {link.label}
                      </Link>
